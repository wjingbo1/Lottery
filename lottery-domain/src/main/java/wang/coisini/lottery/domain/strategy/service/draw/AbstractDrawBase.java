...
import wang.coisini.lottery.domain.strategy.model.vo.*;
...
public abstract class AbstractDrawBase extends DrawStrategySupport implements IDrawExec{
    ...
    public DrawResult doDrawExec(DrawReq req) {
        ...
        List<Long> excludeAwardIds = this.queryExcludeAwardIds(req.getStrategyId());

        // 4. 执行抽奖算法
        Long awardId = this.drawAlgorithm(req.getStrategyId(), drawAlgorithmGroup.get(strategy.getStrategyMode()), excludeAwardIds);
     ...
     * @return 中奖结果
     */
    private DrawResult buildDrawResult(String uId, Long strategyId, Long awardId) {
        if (null == awardId) {
